...
//Declaração da variavel l usada na função insere_meio
	...
	do
	{
		...
		switch(opcao)
		{
			case 1:
				...
				if(i==0) //se for a primeira struct a ser criada, vai ser o inicio, nao preciso entrar em funçao nenhuma;
				{
					inicio->h=hectares;
					inicio->s=sacas;
					inicio->vs=valor_s;
				...
				{
					auxiliar=inicio;
					if(ano<auxiliar->a) //verifico se é o menor ano digitado
					{
					...
					{
						auxiliar=fim;
						if(ano>auxiliar->a) //verifico se é o maior ano digitado
						{
